...
function warnTheSheep(array $animalQueue): string
{
    $animalQueue[] = "me";
    $animalQueue = array_combine(array_reverse(array_keys($animalQueue)), array_values($animalQueue));
    // var_dump($animalQueue);die();

    $wolfPosition = array_search("wolf", $animalQueue);

    if ($wolfPosition === 1) {
        echo "Pls go away and stop eating my sheep!";
    } else {
        echo "Oi! Sheep number " . ($wolfPosition - 1) . "! You are about to be eaten by a wolf!";
    }
    return " (Wolf is in position: " . $wolfPosition . ")";
}
